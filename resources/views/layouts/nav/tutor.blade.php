 ...
 <ul class="nav nav-pills flex-sm-column flex-row flex-nowrap flex-shrink-1 flex-sm-grow-0 flex-grow-1 
        ...
        @if (Auth::user()->tipo_usuario == 'tuniversidad')
            <!-- @if (Route::has('fichaSeg.add'))   
                <li class="nav-item">
                    <a href="{{-- route('fichaSeg.add') --}}" class="nav-link px-sm-0 px-2 text-white nav_link {{ Route::is('fichaSeg.add') ? 'active' : '' }}">
        ...
        @endif
        @if (Route::has('alumnos.tutorHistorial'))
            <li class="nav-item">
                <a href="{{ route('alumnos.tutorHistorial') }}" class="nav-link px-sm-0 px-2 text-white nav_link {{ Route::is('alumnos.tutorHistorial') ? 'active' : '' }}">
